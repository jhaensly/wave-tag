...
def GetTweetsFromTwitter(count, filename='twitter_credentials.p'):
...
if __name__ == '__main__':
    ...
    probability_filename='character_probability.p'
    refresh_tweets = False

    p = CharacterProbability()

    if refresh_tweets:
        DeleteFile(tweets_filename)

        with open(tweets_filename, 'wb') as f:
            count = 0
            target = 1000000
            for tweet in GetTweetsFromTwitter(target):
                count += 1
                if (count % 100) == 0:
        ...
        p.Save(probability_filename)

    else:
        p.Load(probability_filename)
    ...
    plistlib.writePlist(encoder_dictionary, 'encoder_dictionary.plist')

    garbage_value = 255
    arr = h.Serialize(garbage_value)

    datestr = datetime.date.today().strftime('%d %b %Y')
    decoder_path = '../../firmware/'
    decoder_root = 'vlc_decoder_data'

    DeleteFile(decoder_path + decoder_root + '.h')
    DeleteFile(decoder_path + decoder_root + '.c')

    arr_name  = 'VLC_DECODER_DATA'
    numel_name   = arr_name + '_ELEMENTS'
    invalid_name = arr_name + '_INVALID'
    header_grd   = arr_name.upper() + '_H_'

    with open(decoder_path + decoder_root + '.h', 'wb') as f:
        f.write('/**\n')
        f.write(' * @file %s.h\n' % decoder_root)
        f.write(' *\n')
    ...
    with open(decoder_path + decoder_root + '.c', 'wb') as f:
        f.write('/**\n')
        f.write(' * @file %s.c\n' % decoder_root)
        f.write(' *\n')
        f.write(' * @author This file was procedurally generated on %s\n' % datestr)
