...
static void (* m_timer0_cb)(void);
//static void (* m_timer1_cb)(void);


/* Function definitions */
enum error_t timerInit(void)
{
    /*
...
enum error_t timer0Start(void (* timer_cb)(void),
                         uint8_t usec,
                         bool recurring)
{
    if (!timer_cb || (usec == 0)) {
        return ERR_TIMER_INVALID_PARAM;
    } else if (!(PRR & _BV(PRTIM0))) {
        return ERR_TIMER_BUSY;
    }

    m_timer0_cb         = timer_cb;
...
enum error_t timer0Restart(void (* timer_cb)(void))
{
    if (PRR & _BV(PRTIM0)) {
        return ERR_TIMER_STOPPED;
    } else if (!timer_cb) {
...
#if 0
enum error_t timer1Start(void (* timer_cb)(void),
                    uint16_t msec,
                    bool recurring)
{
    if (!timer_cb || (msec == 0)) {
        return ERR_TIMER_INVALID_PARAM;
    } else if (!(PRR & _BV(PRTIM1))) {
        return ERR_TIMER_BUSY;
    }

    m_timer1_cb         = timer_cb;
...
enum error_t timer1Restart(void)
{
    if (PRR & _BV(PRTIM1)) {
        return ERR_TIMER_STOPPED;
    }
